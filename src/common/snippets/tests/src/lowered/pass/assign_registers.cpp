...
AssignRegistersTest::AssignRegistersTest() : LoweredPassTestsF() {
    // Reg info is stored in port descriptors
...
void AssignRegistersRef::set_reg_types(ov::snippets::lowered::LinearIR& linear_ir) {
    using namespace ov::snippets;
    for (const auto& expr : linear_ir) {
        const auto op = expr->get_node();
        if (ov::is_type<ov::snippets::op::LoopEnd>(op) ||
...
bool AssignRegistersRef::run(ov::snippets::lowered::LinearIR& linear_ir) {
    using Reg = size_t;
    using tensor = PortConnectorPtr;
    using RegType = ov::snippets::RegType;
    ...
    for (const auto& result : results) {
        ...
        const auto& shape_infer_sources = ov::snippets::utils::get_first_parent_shape_infer_expr_seq(result);
    ...
    for (const auto& expr : exprs) {
        ...
        if (const auto& buffer = ov::as_type_ptr<ov::snippets::op::Buffer>(op)) {
            const auto reg_group = buffer->get_reg_group();
            // All buffers have one common data pointer
            if (ov::is_type<ov::snippets::op::IntermediateMemoryBuffer>(buffer)) {
                ...
                for (const auto& input : expr->get_input_port_connectors()) {
                    ...
                    const auto& shape_infer_consumers = ov::snippets::utils::get_first_child_shape_infer_expr_seq(expr);
                    for (const auto& child_shape_infer_expr : shape_infer_consumers) {
        ...
        } else if (ov::is_type<ov::snippets::op::HorizonMax>(op) || ov::is_type<ov::snippets::op::HorizonSum>(op)) {
            ...
            for (const auto& tensor : input_expr_input_tensors) {
                ...
                if (ov::is_type<ov::snippets::op::Fill>(parent_expr->get_node())) {
                    if (ov::is_type<ov::snippets::op::VectorBuffer>(parent_expr->get_input_port_connector(0)->get_source().get_expr()->get_node())) {
