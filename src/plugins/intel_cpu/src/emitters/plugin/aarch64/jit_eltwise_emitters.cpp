...
/// NOTEQUAL ///
jit_notequal_emitter::jit_notequal_emitter(dnnl::impl::cpu::aarch64::jit_generator *host,
                                           dnnl::impl::cpu::aarch64::cpu_isa_t host_isa,
                                           const std::shared_ptr<ov::Node>& node)
                                           : jit_emitter(host, host_isa, get_arithmetic_binary_exec_precision(node)) {
    prepare_table();
...
size_t jit_notequal_emitter::get_inputs_count() const { return 2; }

size_t jit_notequal_emitter::get_aux_vecs_count() const { return 1; }

size_t jit_notequal_emitter::get_aux_gprs_count() const { return 1; }

std::set<std::vector<element::Type>> jit_notequal_emitter::get_supported_precisions(const std::shared_ptr<ov::Node>& node) {
    return {{element::f32, element::f32}};
}

void jit_notequal_emitter::emit_impl(const std::vector<size_t>& in_vec_idxs, const std::vector<size_t>& out_vec_idxs) const {
    if (host_isa_ == dnnl::impl::cpu::aarch64::asimd) {
        emit_isa<dnnl::impl::cpu::aarch64::asimd>(in_vec_idxs, out_vec_idxs);
    } else {
        OV_CPU_JIT_EMITTER_THROW("Can't create jit eltwise kernel");
...
template <dnnl::impl::cpu::aarch64::cpu_isa_t isa>
void jit_notequal_emitter::emit_isa(const std::vector<size_t> &in_vec_idxs, const std::vector<size_t> &out_vec_idxs) const {
    OV_CPU_JIT_EMITTER_ASSERT(exec_prc_ == ov::element::f32, "unsupported precision: " + exec_prc_.to_string());
    ...
    h->fcmeq(dst.s, src1.s, src2.s); // Perform equality check
    h->ld1r(aux.s, table_val2("one"));
    h->eor(dst.b16, dst.b16, aux.b16); // Invert the result to get 'not equal'
}

void jit_notequal_emitter::register_table_entries() {
    push_arg_entry_of("one", 0x3f800000, true);
}
